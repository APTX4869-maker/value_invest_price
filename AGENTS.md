...
- SEC + LLM 研究助手：SEC 研究包底座已落地，可抓取 10-K/10-Q 并抽取 Business、Risk Factors、MD&A 等关键章节；下一步在此基础上接 LLM 生成带出处的业务、护城河、竞争和风险初稿，默认待用户确认。
...
- SEC + LLM 研究助手：已完成 SEC filing 文本解析第一版，支持在公司档案内刷新 SEC 研究包并查看关键章节证据。后续接入 OpenAI-compatible Provider，生成带出处的业务、护城河、竞争和风险初稿，并支持确认、编辑、驳回。
...
- SEC 研究包提供 10-K/10-Q 原文证据摘录；后续 LLM 生成的业务、护城河和风险分析只能作为带出处的研究初稿，不能直接作为最终结论；用户确认和结构化 memo 才是后续复盘依据。
