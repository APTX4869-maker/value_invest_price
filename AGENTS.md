...
- 发现雷达：默认扫描 S&P 500，QQQ 和自定义股票池作为可选池，按综合研究优先级寻找可能低于实际价值且值得继续研究的公司。
- 研究队列：承接发现雷达筛出的公司，使用“候选、初筛通过、深度研究中、已形成结论、复盘中”五个状态和多标签管理研究进度。
- 公司研究档案：围绕单家公司整合一页纸结论、业务与护城河、财务质量、估值、同行比较、投资备忘录和历史复盘。
- SEC + LLM 研究助手：抓取 10-K/10-Q 关键章节，生成带出处的业务、护城河、竞争和风险初稿，但默认待用户确认。
- 投资备忘录：从自由笔记升级为结构化 memo，沉淀投资假设、反证清单、估值判断和复盘触发条件。
...
- 发现股票：当前已实现 QQQ 前 30 大权重公司的第一版扫描；后续按 PRD 升级为 S&P 500 默认发现雷达。
- 研究笔记：保存投资假设、风险清单和复盘。
...
- `backend/discovery.py`：当前 QQQ 发现池、候选股评分、护城河/财务/竞争/数据质量原因生成；后续应泛化为 S&P 500 / QQQ / 自定义池通用发现雷达。
...
- 双引擎产品重构：按 `docs/product/value-study-redesign-prd.md`，将当前功能菜单式页面升级为“发现雷达 + 研究队列 + 公司档案 + 投资备忘录 + 历史复盘”。第一阶段先完成 S&P 500 发现雷达 Top 50/Top 100、研究队列 5 状态和公司档案 tabs 整合。
- S&P 500 发现雷达：把 QQQ 前 30 扫描泛化为股票池扫描。默认池改为 S&P 500，保留 QQQ 和自定义池；支持成分缓存、分批扫描、扫描失败解释、综合研究优先级和入队理由。
- 研究队列：把“观察池”产品心智升级为研究队列，支持候选、初筛通过、深度研究中、已形成结论、复盘中五个状态，多标签、下一步动作和入队理由。
- 公司研究档案：把公司分析、估值模型、同行比较、研究笔记和历史快照收敛到同一个 ticker 档案内，顶部固定一页纸结论。
- 结构化投资备忘录：新增 memo 字段，覆盖当前结论、为什么关注、核心投资假设、业务与护城河、财务质量、估值判断、我可能错在哪里、复盘触发条件；保留现有 Markdown 作为自由补充。
- SEC + LLM 研究助手：后续接入 SEC filing 文本解析和 OpenAI-compatible Provider，生成带出处的业务、护城河、竞争和风险初稿，并支持确认、编辑、驳回。
...
- QQQ 发现股票：已完成第一版，左侧新增“发现股票”页；默认扫描 QQQ 前 30 大权重公司，按估值置信度优先、低估幅度其次排序，并展示排名原因、护城河、财务质量、行业竞争风险和数据质量。后续不再把 QQQ 作为默认大盘发现池，而是作为 S&P 500 发现雷达里的可选科技成长子池。
...
- 发现池后续增强：当前第一版只默认扫描 QQQ 前 30。后续按 PRD 升级为 S&P 500 默认发现雷达，可继续补行业热力图、低估幅度 × 可信度矩阵、历史发现记录、批量导出、护城河自定义打分和外部共识数据导入。
- 增加深色模式，但不要牺牲阅读舒适度。
...
- SEC + LLM 生成的业务、护城河和风险分析只是带出处的研究初稿，不能直接作为最终结论；用户确认和结构化 memo 才是后续复盘依据。
