...
- 结构化投资备忘录：已新增 memo 本地存储、前端表单和第一版版本链，覆盖当前结论、为什么关注、核心投资假设、业务与护城河、财务质量、估值判断、我可能错在哪里、复盘触发条件；保留自由补充区。每次手动保存和 AI 初稿确认写入都会记录 `investment_memo_versions`，备忘录页展示最近版本。后续可补版本差异对比。
